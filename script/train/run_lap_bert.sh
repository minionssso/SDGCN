  ...
  do
    ...
    do
      ...
      do
        ...
        do
          gcn_dropout=.40
          while [ `expr .50 \> $gcn_dropout` -eq 1 ]
          do
            mhsa_dropout=.20
            while [ `expr .30 \> $mhsa_dropout` -eq 1 ]
            do
              threshold=.50
                  ...
                  do
                    if [ $idx -ge $start ] && [ $idx -le $end ];then
                        ...
                        python -u train.py --num_epoch 200 --DEVICE 0 --gcn_dropout $gcn_dropout --l2reg $l2reg --mhsa_dropout $mhsa_dropout \
                          --threshold $threshold --lr $lr --dataset Laptops --hidden_dim 204 --rnn_hidden 204 --syn_srd $syn_srd --sem_srd $sem_srd \
                          --num_layers $num_layers --emb_type bert --log_step 40 --head_num $head_num --batch_size 32  \
                          > ./out_lap_bert/$idx.out 2>&1
                    fi
