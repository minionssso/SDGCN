                ...
                do
                  if [ $idx -ge $start ] && [ $idx -le $end ];then
                      ...
                      python -u train.py --num_epoch 200 --DEVICE 0 --gcn_dropout $gcn_dropout --l2reg $l2reg --mhsa_dropout $mhsa_dropout \
                        --threshold $threshold --lr $lr --dataset Laptops --hidden_dim 204 --rnn_hidden 204 --syn_srd $syn_srd --sem_srd $sem_srd --seed 9584 \
                        --num_layers $num_layers --emb_type glove --log_step 40 --head_num $head_num --batch_size 32 --load_model True --load_model_path './saved_models/Glove/Laptops/best_model_lap_77.1875.pt' \
                        > ./out_lap_glove/$idx.out 2>&1
                  fi
